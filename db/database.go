...
// MigrateAndSeed performs database migrations for core and module models,
// seeds default settings, syncs plans, and creates an initial admin user if needed.
// It requires an already initialized *gorm.DB connection.
func MigrateAndSeed(db *gorm.DB) error { // Changed signature, renamed function
	var err error

	// Ensure uuid-ossp extension is enabled (idempotent)
	// Use the passed 'db' directly now
	if err = db.Exec(`CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`).Error; err != nil {
		log.Printf("ERROR: Failed to enable uuid-ossp extension: %v", err)
	...
	// Get module models using the correct package
	moduleModels := models.GetAllModels() // Updated from dbmodels
	...
	// Use the passed 'db' directly now
	if err = models.SeedDefaultSettings(db, models.DefaultSettings); err != nil {
		log.Printf("WARN: Failed to seed default settings: %v", err)
...
// (Function body remains largely the same, already accepts *gorm.DB)
...
// Removed: CloseDatabase function
